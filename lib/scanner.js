...
export async function scanRepository(owner, repo, installationToken, repoId, forceRescan = false) {
  try {
    ...
    console.log(`[scanner] Starting scan for repo_id=${repoIdInt} (${owner}/${repo}), forceRescan=${forceRescan}`);
    ...
    if (!forceRescan) {
      ...
      console.log('[scanner] Force rescan enabled; bypassing scan_cache');
    }
...
${concatenated.slice(0, 30000)}`;
    ...
    console.log(`[scanner] dependency_map updated with ${dependencies.length} rows for repo_id=${repoIdInt}`);
